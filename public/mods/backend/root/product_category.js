    ...
    },
    {
        ...
        mRender: function (data, type, row) {
            return `
                <div class="row">
                    <div class="col-md-2">
                        <img class="round" src="${$('meta[name="asset-url"]').attr('content') + `storage/images/product-categories/${data}`}" alt="avatar" height="40" width="40">
                    </div>
                </div>
            `
        }
...
$('.btn-add').on('click', function () {
    $('#form-modal .modal-title').text('Tambah Kategori Produk');
    resetInvalid();
    $('#product-category-form').trigger('reset');
    $('#product-category-form').attr('action', `${window.location.href}/store`);
    $('.message').html('');
});

function editData(id) {
    $('#form-modal .modal-title').text('Ubah Kategori Produk');
    resetInvalid();
    $('#product-category-form').trigger('reset');
    $('#product-category-form').attr('action', `${window.location.href}/${id}/update`);
